...
#pragma endregion
    ...
    for (int i = 0; i < 60; i++)
    {
        float radius = randomf(0.3, 0.7f);
        auto sphere = std::make_unique<Sphere>(Transform{ glm::vec3(randomf(-10, 10), (-2 + radius), randomf(-10, 10))}, radius, materials.at(random(materials.size())));
    ...
    auto triangle = std::make_unique<Triangle>(glm::vec3{ -8, -2, 10 }, glm::vec3{ 0, 14, 10 }, glm::vec3{ 8, -2, 10 }, materials.at(25));
    ...
    auto sphere = std::make_unique<Sphere>(Transform{ glm::vec3{0, 0, -10} }, 1.0f, materials.at(32));
    scene.AddObject(std::move(sphere));
}
